...
func (a *Agent) Compact(ctx context.Context) error {
	...
	if err := a.messages.ReplaceAll([]*Message{summaryMsg}); err != nil {
		return err
	}
	a.eventBroker.Publish(AgentEvent{Type: EventCompact})
	return nil
}
...
func cleanHistory(messages []*Message) []*Message {
	...
	for i := 0; i < len(messages); i++ {
		...
		for _, id := range callIDs {
			...
			out = append(out, toolNoResponse(id))
		}
		i = next - 1
	}
	if len(out) > 0 && out[len(out)-1].Role == RoleTool {
		return append(out, assistantFollowup())
	}
	return out
}
...
func extractToolResults(msg *Message, pending map[string]struct{}) *Message {
	out := &Message{ID: msg.ID, Role: msg.Role, CreatedAt: msg.CreatedAt}
	for _, part := range msg.Parts {
...
func toolNoResponse(callID string) *Message {
	return &Message{
		ID:        uuid.NewString(),
		Role:      RoleTool,
...
func assistantFollowup() *Message {
	return &Message{
		ID:        uuid.NewString(),
		Role:      RoleAssistant,
