...
func TestGrepToolFindsPattern(t *testing.T) {
	...
	if !strings.Contains(got.Content, "a.txt:2:match line") {
		t.Fatalf("missing expected result in %q", got.Content)
	}
}
...
func TestGrepToolSupportsRegex(t *testing.T) {
	...
	for _, want := range []string{"a.txt:1:foo1", "a.txt:2:foo2"} {
		if !strings.Contains(got.Content, want) {
...
func TestGrepToolSkipsBinary(t *testing.T) {
	...
	if !strings.Contains(got.Content, "text.txt:2:needle") {
...
func TestGrepToolRespectsGitignore(t *testing.T) {
	...
	if !strings.Contains(got.Content, "kept.txt:1:needle") {
...
func TestGlobToolSimplePattern(t *testing.T) {
	...
	sort.Strings(lines)
	if len(lines) != 2 || lines[0] != "a.txt" || lines[1] != "sub/c.txt" {
